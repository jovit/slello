...
class Group extends React.Component {
    ...
    async componentDidMount() {
        ...
        const communitiesResponse = await axios.get(`${baseUrl}/v1/communities/?path=${this.state.communityUrl}`, {
            data: {},
            headers: {
        ...
        const topicsResponse = await axios.get(`${baseUrl}/v1/topics/?communityId=${community.id}`, { headers: { Authorization: `Bearer ${token}`, 'Content-type': 'application/json' } })
    ...
    return(
            ...
            {
              (this.state.topics || []).sort((a, b) => b.votes - a.votes).map( (discussion, index) => (
                <Feed.Event className="feed-event">
                  <Feed.Content className="feed-event-content">
                    ...
                    <Feed.Meta>
                      ...
                      <Link to="#">
                        <Icon name="talk" />
                        { discussion.comments.length } { discussion.comments.length !== 1 ? 'comments' : 'comment' }
                      </Link>
                      <Feed.Like>
